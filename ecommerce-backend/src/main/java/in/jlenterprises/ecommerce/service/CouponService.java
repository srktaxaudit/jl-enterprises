...
public interface CouponService {
    ...
    List<CouponDto> eligibleFor(UUID userId);
    ...
    CouponValidationResult validate(String code, UUID userId);

    /** Validate and return the entity + computed discount, for order placement. */
    AppliedCoupon apply(String code, List<CartItem> items, UUID userId);
    ...
    record AppliedCoupon(Coupon coupon, BigDecimal discount, BigDecimal eligibleSubtotal,
                         List<in.jlenterprises.ecommerce.dto.coupon.CouponItemEligibility> items) {}
}
